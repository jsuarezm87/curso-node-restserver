...
const { subirArchivo, existeArchivo } = require('../helpers');
...
const actualizarImagen = async ( req, res = response) => {
    ...
    switch ( coleccion ) {
        case 'usuarios':
            ...
            if ( !modelo ) return res.status(400).json({ msg: `No existe usuario ${ id }`});            
        break;
        case 'productos':
            modelo = await Producto.findById( id );
            if ( !modelo ) return res.status(400).json({ msg: `No existe producto ${ id }`});        
        break;    
        default:
